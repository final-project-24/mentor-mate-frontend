...
export default function AdminTools() {
  ...
  const linkClass = classNames('p-3 bg-accent text-primary text-center rounded-md transition', {
      ...
      <>
        {isLoggedIn &&
          (user.role === "admin" || user.originalRole === "admin") && (
            <div className="w-full md:w-[80%] xl:w-[50%] flex flex-col gap-3 bg-card p-3 border-2 rounded-lg">
              <Link
                className={linkClass}
              ...
              <Link
                className={linkClass}
                to="/dashboard/admin-tools/skills"
              >
                Manage Skills
              </Link>
            </div>
